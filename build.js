#!/usr/bin/env node
/**
 * Build standalone binaries for klonk using bun compile.
 *
 * Usage:
...
function build(name) {
  ...
  const outDir = join(DIST, `klonk-${name}`);
  const outFile = join(outDir, `klonk${ext}`);
...
console.log("🔊 Building klonk standalone binaries\n");
